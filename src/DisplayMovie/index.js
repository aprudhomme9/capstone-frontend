...
class DisplayMovie extends Component {
  ...
  handleRecSubmit = async () => {
    if (this.props.user) {
      ...
      const userToEdit = await fetch(
        serverUrl + "api/users/" + this.state.userId,
        { credentials: "include" }
      );
  ...
  /**************************
	Get movie from props, edit group by creating new array of movies
	Handles duplicates via filter()
	**************************/
  handleSubmit = async () => {
    const movieToAdd = this.props.movie;

    const groupToEdit = await fetch(
      serverUrl + "api/groups/" + this.state.groupId
    );
    ...
    // Closes modal for adding to group
    this.toggleModal();
    // Toggles original modal when selecting movie via props
    this.props.toggleView();
  };
  // Toggles modal for recommending
  toggleRecModal = () => {
    this.setState({
      recOpen: !this.state.recOpen
    });
  };
  // Toggles modal for adding to group
  toggleModal = async e => {
    this.setState({
  ...
  addToFavorites = async e => {
    if (this.props.user) {
      e.preventDefault();
      const movieToAdd = await fetch(
        serverUrl + "api/movies/movie/add/" + this.props.movie._id
      );
      const parsedMovie = await movieToAdd.json();
      const userId = this.props.user._id;
      const userToEdit = await fetch(serverUrl + "api/users/" + userId, {
        credentials: "include"
      });
      ...
      const updatedMovie = await fetch(
        serverUrl + "api/movies/movie/" + this.props.movie._id,
        {
          method: "PUT",
          credentials: "include",
          body: JSON.stringify({
    ...
    return (
      <div>
        <Modal open={this.state.open}>
          ...
          <Modal.Content>
            <p className="close" onClick={this.toggleModal}>
              +
            </p>
            <Dropdown
        ...
        <Modal open={this.state.recOpen}>
          ...
          <Modal.Content>
            <p className="close" onClick={this.toggleRecModal}>
              +
            </p>
            <Dropdown
